...
┌─────────────────────────────────────────┐
│              FastAPI (api/)             │
│   /analyze/stream  /datasets  /schema   │
└────────────────────┬────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────┐
...
│   ├── graph/          # LangGraph 状态机和路由；language.py（detect_language / 可选 langdetect）
│   ├── tools/          # 所有可调用工具 + 注册表
│   ├── memory/         # 三层记忆系统
...
### Dataset Panel 恢复能力总览（新增）

为支持调试场景下的快速复用，系统新增了 Dataset Panel 的「磁盘可恢复」能力。目标是：即使前后端重启，用户仍可直接加载已有 DuckDB 项目，而不必重复上传文件。

恢复能力由三层协作完成：

1. 前端 Dataset Panel 提供 debug 入口，查询可恢复项目列表并触发加载
2. API 层提供 `GET /api/debug/projects` 用于发现本地 `proj_*.duckdb`
3. 后端在 `schema` 和 `analyze` 路由中内置 bootstrap：当项目不在内存态时，自动从 `data/processed/{project_id}.duckdb` 重建轻量项目状态
...
UploadZone(debug)
  ...
  │      └─ 扫描 data/processed/proj_*.duckdb
  │
  ├─ 选择 project_id
  │
  ├─ GET /api/projects/{project_id}/schema
  │      └─ 若内存无该项目：_bootstrap_project_from_db(project_id)
  │
  └─ 前端恢复 datasets / strategy_version / system_mode
       ├─ system_mode=chat → panel 默认收起
       └─ 其他模式       → panel 保持展开
```

该设计将「上传阶段扫描信息」与「恢复阶段最小可用信息」解耦：恢复阶段只保证可分析所需核心结构（表、列、行数、确认状态、策略版本），不强制还原上传当时的全部中间诊断细节。

### Dataset Panel 与清洗策略后续扩展（2026-03-26）

在恢复能力上线后，系统继续向“分析师可控”方向扩展，主要体现在三类能力：

1. 模式化清洗策略：simple 与 advanced
  - simple：低摩擦自动策略
  - advanced：更严格的 must-solve 门禁
2. warning 治理从列级转为 issue 级
  - 以 `column:issue_type` 作为决策键
  - 避免同列多 warning 时的覆盖冲突
3. must-solve 阈值与门禁闭环
  - missing 阈值按模式生效（advanced 5%，simple 50%）
  - outlier 占比阈值 8%
  - 前端 confirm 按未解决 must-solve 项阻塞
  - 后端 confirm 再次强校验，避免前端绕过

该扩展使数据清洗链路从“可选提示”演进为“风险分级 + 前后端双重约束”的一致性模型。

一次用户提问从发出到返回的完整流程：

```
1. 用户在分析项目中输入问题，附带当前数据集 ID
2. GET /analyze/stream — 建立 SSE 连接
3. `sse.run_analysis_stream` 对 `query` 调用 `detect_language()`（`backend/graph/language.py`，可选依赖 `langdetect`；
   含足够汉字时用汉字子串再检测，避免中英混合问法误判）。得到 BCP-47 `user_lang`（如 zh-cn、en），写入 `initial_state`，服务端日志打印 `user_lang=...`
4. API 层从 Memory Manager 读取短期上下文和用户偏好（若已接入）
5. 组装初始 AgentState（含 `user_lang`），交给 LangGraph pipeline
6. Planner Agent 拆解任务，决定激活哪些 worker agents
7. SQL Agent 先行执行：
     - 自主生成 SQL，执行查询，处理报错并修正
     - 通过 create_sql_tools(conn) 创建绑定连接的 run_query 工具
8. SQL 结果就绪后，Viz Agent 和 Stats Agent 顺序执行：
     - Viz Agent    → 基于 sql_result 选择图表类型，_build_series_from_rows 确定性构建 series
     - Stats Agent  → 基于 sql_result 执行统计检验（条件激活）
9. 各 worker 逐步更新 AgentState
10. Critic Agent 审核结论：
     - 通过 → Report Agent 整合最终输出
     - 不通过 → 带原因打回指定 worker 重试（最多 2 次）
11. Report Agent 判断本次结果是否值得记录（见第 5 节），
    生成结论文本（Markdown 段落/列表，与 Chart→Table 视图分工；DATA_FACTS 对齐；`user_lang`），
    并决定是否追加到分析项目的 report_records
12. SSE 将每个节点的进度事件和最终结果流式推送到前端
13. Memory Manager 将本次分析结果写入长期记忆，更新用户偏好（若已接入）
```
...
| Agent            | 职责                                                                                                                                                                                      | 可调用的工具                                                                            |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `Planner`      | 理解用户意图，拆解为子任务，决定激活哪些 worker；自然语言输出遵循 `user_lang`（prompt 内 `{user_lang}`）                                                                                  | 无（只做推理）                                                                          |
| `SQL Agent`    | 自主生成 SQL，执行查询，处理报错并修正；预检 + intent contract + 本地 DuckDB RAG；结果自检中 **跳过** 对 critic/metacritic/avg_score 等「稀疏评分列」的 NULL 比例误判；摘要语言遵循 `user_lang` | `run_query`                                                                             |
| `Viz Agent`    | 选择图表类型与标题/轴标签（语言遵循 `user_lang`）；`_build_series_from_rows`、`_compatible_chart_types`、`_is_histogram_intent` 等同上                                                     | 无（纯推理 + 确定性构建）                                                               |
| `Stats Agent`  | 统计检验与异常值；描述字段语言遵循 `user_lang`                                                                                                                                            | 无（LLM 决定测试计划，Python 直接用 scipy 执行）                                        |
| `Critic Agent` | 审核与重试决策；`feedback` 语言遵循 `user_lang`                                                                                                                                           | 无（只做推理）                                                                          |
| `Report Agent` | 生成 Markdown **文字提炼**（列表/加粗；**不**重复 Chart 面板 Table 中的数据网格）；DATA_FACTS 对齐排名与数值；`user_lang`；可选脚注                                        | 无                                                                                      |
...
- **图表**：Viz 侧 **`_build_series_from_rows`** 确定性映射 SQL 行 → series；**`_compatible_chart_types`** 过滤无意义的 `alt_types`；Critic **`_force_histogram_retry`** 拦截「要直方图却只返回极少行」的 SQL。SQL 侧对窗口/排名的契约与结果自检已 **放宽**，并在用尽重试时 **回退到最后一次执行成功的查询**（见 `sql_agent.py`）。
- **结论**：Report 注入 **`ranked_data_facts`（DATA_FACTS）**，正文 **服从 DATA_FACTS** 并限制「一直 / consistently」等表述；**完整表由 Chart 面板 Table 展示**，报告内不重复管道表，避免与 UI 冲突。

**完整条目表与文件定位**：`agent_design.md` **§1.2、§1.3** 及 §4 / §6 / §7。
...
class AgentState(TypedDict, total=False):
    # 输入
    user_query: str
    user_lang: str               # BCP-47，pipeline 入口 detect_language(query)；各 agent 写回复语言
    session_id: str
    project_id: str

    # 数据集上下文
    active_tables: list[dict]     # 当前项目已确认的表信息（名称、列、行数）
    quality_notes: list[str]      # 已应用的清洗决策描述

    # Planner 输出
    plan: list[str]               # 激活的 agent 列表，如 ["sql", "viz"]
    sql_task: str                 # SQL Agent 的任务描述
    viz_task: str | None          # Viz Agent 的任务描述（可选）
    stats_task: str | None        # Stats Agent 的任务描述（可选）
    involved_columns: list[str]   # 本次分析涉及的列名

    # Worker 输出
    sql_result: dict              # {"steps", "final_rows", "final_columns", "error", "quality_warning", "intent_contract"}
    viz_result: dict | None       # {"type", "series", "table_data", ...}
    stats_result: dict | None     # {"tests", "outliers", "summary"}
                                  # tests 非空时 → Report Agent 生成 evidence section

    # Critic 输出
    critic_verdict: str           # "pass" | "retry"
    critic_feedback: str          # 打回时的具体原因
    retry_count: int              # 当前重试次数（上限 2）
    retry_target: str | None      # "sql" | "planner" | "both"

    # 最终输出
    report: dict | None           # 结构化报告
    should_record: bool
    stream_events: Annotated[list[dict], add]  # 累积合并的进度事件列表
```

`total=False` 让所有字段可选——pipeline 入口只需提供 `user_query` 和 `project_id`。`stream_events` 使用 LangGraph 的 `add` reducer，每个节点返回的事件自动追加而非替换。
...
### Results Panel：5 个 Tab

默认显示 Chart tab。

| Tab     | 内容                                 | 说明                         |
| ------- | ------------------------------------ | ---------------------------- |
| Dataset | 数据集预览、编辑、版本管理           | 可编辑表格 + 版本快照        |
...
* **备选类型由后端程序化过滤决定** ：Viz Agent 的 `_compatible_chart_types()` 根据数据形状（x 分类/数值/时序、y 数值、unique x 数量）计算兼容类型，再与 LLM 建议取交集。例如 histogram 分箱数据（x 为 `"0-500"` 字符串）只保留 `bar`/`pie`，不显示无意义的 `scatter`/`line`/`area`。
* **`table` 条件显示** ——当结果行数 ≤ 50 时显示 `table` 选项（面向截图/报告用途），超过 50 行时自动隐藏。
...
```
事件类型：
  progress         — agent 开始/完成某个节点（各 agent 自行放入 stream_events）
  result           — 某个 worker 的中间结果（SQL 步骤等，各 agent 自行放入 stream_events）
  done             — pipeline 完整执行完毕，包含最终结果
                     （report + dataset_versions + sql_result + viz_result + stats_result）
  error            — pipeline 执行异常
```

注：`done` 事件由 `sse.py` 在 pipeline 执行完成后统一构建，包含所有最终结果。中间事件来自各 agent 节点放入 `stream_events` 的数据。
...
**为什么图表类型切换由程序化过滤而非纯 LLM 决定？**
不是所有图表类型对所有数据都有意义。趋势数据可以是折线图/面积图/柱状图，但饼图就不合理；histogram 分箱数据（x 为 `"0-500"` 等分类字符串）不适合 scatter/line。最初依赖 LLM 推荐 `alt_types`，但 LLM 经常输出不兼容的选项。现在 `_compatible_chart_types()` 基于实际数据形状（x 分类/数值/时序、y 数值、unique x 数量）程序化计算兼容列表，再与 LLM 建议取交集。`table` 选项在行数 ≤ 50 时显示（面向截图/报告用途），超过 50 行自动隐藏。
...
运行时切换（无需重启）：
```
PUT /api/llm/provider?provider=ollama&model=qwen3:8b
GET /api/llm/status
```

支持三种 provider：`deepseek`、`ollama`、`anthropic`。
