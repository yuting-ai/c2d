...
function renderCenteredYAxisLabel(props: any) {
  ...
  // Keep label inside axis box to avoid clipping when chart margins are tight.
...
function ChartEntry({
  ...
  const canSwapAxes = rec.activeType === 'line' || rec.activeType === 'area' || rec.activeType === 'bar' || rec.activeType === 'scatter'
  const tableRowCount = rec.tableData?.rows?.length ?? 0
  const canShowTable = tableRowCount > 0 && tableRowCount <= TABLE_VIEW_MAX_ROWS

  useEffect(() => {
    setSwapAxes(false)
  }, [rec.id, rec.activeType])

  useEffect(() => {
    if (rec.activeType === 'table' && !canShowTable) {
      switchView(rec.id, rec.type)
    }
  }, [rec.activeType, canShowTable, rec.id, rec.type, switchView])

  const allTypes = useMemo(() => {
    const types = [rec.type, ...rec.altTypes.filter(t => t !== rec.type)]
    if (canShowTable && !types.includes('table')) types.push('table')
    return types
  }, [rec.type, rec.altTypes, canShowTable])
  ...
  const exportChart = () => {
    if (rec.activeType === 'table') {
      ...
      // Inject title into exported SVG and push chart down slightly.
      const titleText = (rec.title || rec.query || '').trim()
      if (titleText) {
        const headerH = 44

        // Expand viewBox height to make room for a header band.
        ...
        // Move existing graphics down by header height (leave defs and background in place).
        ...
        header.setAttribute('fill', '#09090b')
        ...
        divider.setAttribute('stroke', '#1f2128')
    ...
    <div style={{
      ...
      <div
        ...
        style={{
          ...
          padding: expanded ? '14px 12px 14px 10px' : '13px 12px 13px 10px', cursor: 'pointer',
      ...
      >
        ...
        <span style={{ fontFamily: 'var(--mono)', fontSize: 11.5, color: expanded ? 'var(--text1)' : 'var(--text2)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', opacity: expanded ? 1 : 0.85 }}>
          {rec.title || rec.query}
        </span>
        <span style={{ fontSize: 10, color: 'var(--text3)', transition: 'transform 0.2s', transform: expanded ? 'rotate(90deg)' : '', opacity: 0.7 }}>▶</span>
      </div>
        ...
        <div style={{ padding: '0 12px 12px 12px' }}>
          ...
          <div ref={chartRef}>
            {rec.activeType === 'table' ? (
              ...
              <ChartRenderer record={rec} swapAxes={swapAxes} />
            )}
          </div>
            </>
          )}
...
function ChartRenderer({ record: rec, swapAxes }: { record: ChartRecord; swapAxes: boolean }) {
  ...
  const chartData = useMemo(() => {
    ...
    // Build unified x-axis from all series, then align each series by x value.
    ...
    // Normalize numeric-like strings so XAxis `type="number"` works.
    const normalizeX = (x: string | number): string | number => {
      const n = toNum(x)
      return n === null ? x : n
    }
    const normalizedXValuesRaw = xValues.map((x) => normalizeX(x))
    // Deduplicate after normalization to avoid both "2016" and 2016 coexisting.
  ...
  const commonProps = {
    // Keep data source consistent across line/area/bar.
    data: (rec.activeType === 'line' || rec.activeType === 'area' || rec.activeType === 'bar') ? sampledSeriesData : activeData,
    margin: {
      top: 8,
      right: 16,
      // Bar charts need extra room for axis labels; otherwise labels can be clipped.
...
function InteractiveLegend({
  ...
  const handleLegendClick = (name: string) => {
    ...
    // Custom double-click: same legend key within window => focus action.
    ...
    // Delay single toggle to wait for a potential second click.
